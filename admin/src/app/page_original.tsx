...
function AccordionItem({ title, content }: { title: string; content: string }) {
  ...
  return (
    <div className="border dark:border-zinc-800 rounded-lg overflow-hidden bg-white dark:bg-zinc-950">
      ...
      >
        <span>{title}</span>
        <span className="text-zinc-400">{isOpen ? 'ΓêÆ' : '+'}</span>
      </button>
      {isOpen && (
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.user.roles?.some((r: string) => ['Admin', 'Super Admin', 'Manager'].includes(r)))
        throw new Error('Access denied ΓÇö admin role required');
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          {activeTab === 'users' && (
            <Section>
              ...
              <FilterBar>
                <SearchBar value={userSearch} onChange={setUserSearch} placeholder="Search email, name, phoneΓÇª" />
            ...
            <Section>
              {selectedOrder ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border dark:border-zinc-800 space-y-1 text-zinc-700 dark:text-zinc-300">
                          <div><strong>Name:</strong> {selectedOrder.shippingAddress?.fullName || 'ΓÇö'}</div>
                          <div><strong>Street:</strong> {selectedOrder.shippingAddress?.addressLine1 || 'ΓÇö'}</div>
                          <div><strong>City/State:</strong> {selectedOrder.shippingAddress?.city || 'ΓÇö'}, {selectedOrder.shippingAddress?.state || 'ΓÇö'}</div>
                          <div><strong>Postal Code:</strong> {selectedOrder.shippingAddress?.postalCode || 'ΓÇö'}</div>
                          <div><strong>Country:</strong> {selectedOrder.shippingAddress?.country || 'ΓÇö'}</div>
                          <div><strong>Phone:</strong> {selectedOrder.shippingAddress?.phone || 'ΓÇö'}</div>
                        </div>
                      </div>
                    </div>
                        ...
                        {selectedOrder.items?.map((item: any, idx: number) => (
                          <div key={idx} className="flex justify-between items-center py-2 text-xs first:pt-0 last:pb-0">
                            <div>
                              ...
                              <div className="text-[10px] text-zinc-400 font-mono">Qty: {item.quantity} ├ù ${item.price?.toFixed(2)}</div>
                            </div>
                ...
                <>
                  ...
                  <FilterBar>
                    <SearchBar value={orderSearch} onChange={setOrderSearch} placeholder="Search order ID, statusΓÇª" />
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                          {paginatedOrders.map((o: any) => (
                            <tr key={o._id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                              ...
                              <td className="px-4 py-3 text-xs text-zinc-400 font-mono">{o.trackingCode || 'ΓÇö'}</td>
                              <td className="px-4 py-3 text-xs text-zinc-400">{o.createdAt ? new Date(o.createdAt).toLocaleString() : 'ΓÇö'}</td>
            ...
            <Section>
              {selectedProduct ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t dark:border-zinc-800">
                          <div className="space-y-1">
                            ...
                            <span className="font-bold text-sm text-zinc-900 dark:text-white flex items-center gap-1">
                              Γÿà {selectedProduct.averageRating || 0}
                            </span>
                          </div>
                          <div className="space-y-1">
                              ...
                              <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-3">
                                  ...
                                  <div>
                                    ...
                                    <span className="font-bold text-zinc-800 dark:text-zinc-200">{matchedVendor.companyLegalName || 'ΓÇö'}</span>
                                  </div>
                                  <div>
                                    <span className="text-[10px] text-zinc-400 block font-medium">Contact Phone</span>
                                    <span className="font-bold text-zinc-800 dark:text-zinc-200">{matchedVendor.businessPhone || 'ΓÇö'}</span>
                                  </div>
                                  <div>
                                    <span className="text-[10px] text-zinc-400 block font-medium">Associated Account</span>
                                    <span className="font-bold text-zinc-800 dark:text-zinc-200">{matchedVendor.userId?.email || 'ΓÇö'}</span>
                                  </div>
                                </div>
                                <button
                                  onClick={() => {
                ...
                <>
                  ...
                  <FilterBar>
                    <SearchBar value={productSearch} onChange={setProductSearch} placeholder="Search product titleΓÇª" />
          ...
          {activeTab === 'vendors' && (
            <Section>
              ...
              <FilterBar>
                <SearchBar value={vendorSearch} onChange={setVendorSearch} placeholder="Search shop, companyΓÇª" />
                      ...
                      {paginatedVendors.map((v: any) => {
                        ...
                        return (
                          <tr key={v._id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                            ...
                            <td className="px-4 py-3 text-sm text-zinc-500">{v.userId?.email || 'ΓÇö'}</td>
                            <td className="px-4 py-3 text-sm text-zinc-500">{v.companyLegalName || 'ΓÇö'}</td>
                            <td className="px-4 py-3 text-sm text-zinc-500">{v.businessPhone || 'ΓÇö'}</td>
                                ...
                                return (
                                  <tr key={t._id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 text-xs">
                                    ...
                                    <td className="px-4 py-3 text-zinc-400">
                                      {t.createdAt ? new Date(t.createdAt).toLocaleString() : 'ΓÇö'}
                                    </td>
                                  </tr>
                                );
                              })}
                        ...
                        {selectedFeedback.rating > 0 && (
                          <div className="bg-emerald-50 border border-emerald-250 text-emerald-800 dark:bg-emerald-950/20 dark:border-emerald-900/30 dark:text-emerald-450 p-5 rounded-2xl shadow-sm space-y-2">
                            ...
                            <div className="flex items-center gap-1">
                              {[1, 2, 3, 4, 5].map(star => (
                                <span key={star} className={`text-lg ${star <= selectedFeedback.rating ? 'text-amber-500' : 'text-zinc-300'}`}>Γÿà</span>
                              ))}
                            ...
                            {feedbackTickets.map((t: any) => {
                              return (
                                <tr key={t._id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 text-xs cursor-pointer" onClick={() => loadFeedbackDetail(t._id)}>
                                  ...
                                  <td className="px-4 py-3">{t.roadmapStatus !== 'none' ? badge('indigo', t.roadmapStatus) : 'ΓÇö'}</td>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl border dark:border-zinc-800 p-6 space-y-4 shadow-sm">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-emerald-500 font-bold mt-1">Γåæ +14.2% from last month</p>
                      </div>
                      {/* Sparkline */}
                      <div className="h-10">
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl border dark:border-zinc-800 p-6 space-y-4 shadow-sm">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-emerald-500 font-bold mt-1">Γåæ +8.3% vs yesterday</p>
                      </div>
                      <div className="h-10">
                        <svg className="w-full h-full" viewBox="0 0 100 30" preserveAspectRatio="none">
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl border dark:border-zinc-800 p-6 space-y-4 shadow-sm">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-emerald-500 font-bold mt-1">Γåæ +2.4% avg conversion price</p>
                      </div>
                      <div className="h-10">
                        <svg className="w-full h-full" viewBox="0 0 100 30" preserveAspectRatio="none">
          ...
          {activeTab === 'health' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button 
                  onClick={() => {
                    alert("System Health Diagnostic Complete:\n\nΓÇó CPU Cores: 12 Cores Online\nΓÇó NestJS Backend REST API: Healthy\nΓÇó MongoDB Atlas Cluster: 9ms Latency\nΓÇó Redis Caching Server: Active\nΓÇó Socket.IO Live Support Server: Active\nΓÇó Elasticsearch Indexing: 100% Synced");
                  }}
...
function Loading() {
  return <div className="px-6 py-10 text-sm text-zinc-400 flex items-center gap-2"><RefreshCw className="h-4 w-4 animate-spin" /> LoadingΓÇª</div>;
}
...
function SearchBar({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div className="relative flex-1 min-w-[200px]">
      ...
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder ?? 'SearchΓÇª'}
